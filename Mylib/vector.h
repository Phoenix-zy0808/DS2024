...
template <typename T> int Vector<T>::deduplicate() // 删除无序向量中重复元素(高效版)
{
    ...
    while (i < _size)  // 自前向后逐一考查各元素_elem[i]
      ...
      i++: remove(i); // 若无雷同则继续考察后继，否则删除雷同者
    return oldSize - _size; // 向量规模变化量，即被删除元素总数
}

// 2.15向量遍历接口traverse()
template <typename T> void Vector<T>::traverse(void (*visit)(T &)) // 利用函数指针机制遍历
{ for (int i = 0; i < _size; i++) visit(_elem[i]);}//遍历变量
    ...
    while (lo < hi) // 每步迭代可能要做两次判断，有三个分支
{
    ...
    if (e < A[mi]) hi = mi; // 深入前半段[lo,mi]继续寻找
